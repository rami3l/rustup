...
impl MockChannel {
    pub(super) fn new(
        ...
        if multi_arch {
            ...
            let target_tuple = MULTI_ARCH1.to_string();
            all.std.push((std, target_tuple.clone()));
            all.rustc.push((rustc, target_tuple.clone()));
            all.cargo.push((cargo, target_tuple.clone()));

            if rls != RlsStatus::Unavailable {
                let rls = MockInstallerBuilder::rls(version, version_hash, rls.pkg_name());
                all.rls.push((rls, target_tuple.clone()));
            } else {
                all.rls.push((
                    MockInstallerBuilder { components: vec![] },
                    target_tuple.clone(),
            ...
            all.docs.push((rust_docs, target_tuple.to_string()));
            all.combined.push((rust, target_tuple));
        }
        ...
        let packages = all.into_iter().map(|(name, target_pkgs)| {
            let target_pkgs =
                target_pkgs
                    .into_iter()
                    .map(|(installer, target_tuple)| MockTargetedPackage {
                        target: target_tuple,
